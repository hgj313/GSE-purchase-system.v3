...
export const useOptimizationResults = (
  ...
  const totalStats = useMemo((): TotalStats => {
    ...
    // ✅ 关键修复：优先使用completeStats中的预计算数据，添加有效性检查
    if (results.completeStats?.global && results.completeStats.global.totalModuleCount > 0) {
  ...
  const { hasDataError, errorMessage } = useMemo(() => {
    ...
    // 检查数据合理性 - 仅在无解决方案时检查模数用量
    if (totalStats.totalModuleCount === 0 && totalStats.totalModuleLength === 0 && (!results.solutions || Object.keys(results.solutions).length === 0)) {
      return {
        hasDataError: true,
        errorMessage: '模数钢材用量为0，可能存在数据异常'
      };
    }
  ...
  const startPolling = useCallback((taskId: string) => {
    ...
    const pollTaskStatus = async () => {
      try {
          ...
          console.log('📦 轮询响应结果:', result);

        // 直接处理任务状态，不依赖result.status === 'success'判断
        // 直接处理任务状态，使用函数式更新确保获取最新状态
        setCurrentTask((prev) => ({ 
            ...prev,
    ...
    try {
      ...
      const backendData = {
        ...
        constraints: {
          ...
          maxWeldingSegments: optimizationData.constraints.maxWeldingSegments + 1
        }
      };
      ...
      if (result.success && result.taskId) {
        ...
        return { success: true, taskId: result.taskId || null };
      } else {
        console.error('❌ 任务创建失败 - 完整响应:', result);
