...
const DEFAULT_CONSTRAINTS = {
  wasteThreshold: 600,        // 废料阈值 (mm) - 余料长度小于此值时视为废料，默认值600mm
  targetLossRate: 5,          // 目标损耗率 (%) - 优化目标参考值（软约束）
  timeLimit: 300000,           // 计算时间限制 (ms) - 算法最大运行时间 = 300秒
...
const VALIDATION_LIMITS = {
  wasteThreshold: {
    min: 100,               // 最小废料阈值 (mm) - 必须≥100mm
    max: 2000,              // 最大废料阈值 (mm) - 放宽上限
    recommended: {
      min: 100,           // 推荐最小值
      max: 1000           // 推荐最大值
    }
...
const REMAINDER_CONFIG = {
  ...
  defaultWasteThreshold: 600,   // 余料管理器默认废料阈值（与DEFAULT_CONSTRAINTS.wasteThreshold保持一致）
...
const PERFORMANCE_CONFIG = {
  parallelOptimization: {
    ...
    optimizerTimeout: 300,   // 优化器超时时间 (ms)
