...
class TaskManager {
  ...
  async createPendingTask(optimizationData, taskId = null) {
    await this.initialize();
    const id = taskId || this.generateTaskId();
    await this.sql`
      INSERT INTO optimization_tasks (id, type, status, progress, message, input_data) 
      VALUES (${id}, 'optimization', 'pending', 0, '任务已创建，等待后台处理', ${JSON.stringify(optimizationData)})`;
    console.log(`✅ 创建待处理任务: ${id}`);
    return id;
  }
